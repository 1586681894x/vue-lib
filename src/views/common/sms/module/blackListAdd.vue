...
<script>
    export default {
        ...
        methods: {
            ...
            loadList(){
                if(this.row.id){
                  this.api('commonSmsBlackList.info',this.row.id).then(({data})=>{
            ...
            formSubmit(){
                this.$refs.form.validate().then((res)=>{
                    if(res.id){
                        this.api('commonSmsBlackList.edit',res).then(response => {
                            this.msgSuccess("修改成功");
                            this.onClose();
                            this.onEnd();
                        });
                    }else{
                        this.api('commonSmsBlackList.add',res).then(response => {
    ...
    export function getColumns(){
        return [
            {name:'mobile',text:'手机号',type:'input',size:24},
            {name:'status',text:'状态',type:'select',value:'enable',options:'smsStatuss',size:24},
            {name:'description',text:'描述',type:'textarea',props:{autosize:{ minRows: 2, maxRows: 6 }},size:24},
        ]
    }
    ...
    export function getRules(){
        return {
            mobile:_.getRules(["required"]),
            status:_.getRules(["required"])
        }
    }
