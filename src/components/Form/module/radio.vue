<template>
  <el-form-item class="form_radio" :label="opt.text" size="mini" v-if="!opt.hide" style="padding-bottom:1px;" >
    <el-radio-group v-model="data[opt.name]" size="mini"
                    v-bind="opt.props"
    ...
    initColumn(){
      ...
      let optionValue = (list)=>{
        ...
        if(_.isNil(nowValue) && list.length){
          column.value = list[0][this.opt.optionsValue];
        }else{
          //
        }
