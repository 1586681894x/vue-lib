<template>
  <el-form-item :label="opt.text" :prop="opt.name" v-if="!opt.hide" size="mini">
    <el-upload class="upload_file"
      ...
      <div v-if="data[opt.name]" class="img_box" style="" :style="`background-image:url(${data[opt.name]})`">
        <div class="fl-center"><i class="iconfont iconxiugai"/></div>
      </div>
      <el-button v-else v-show="!disabled" type="primary" plain icon="el-icon-upload" circle size="mini" ></el-button>
    </el-upload>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            onPreview(){
                this.DialogPreview(this.data[this.opt.name],this.opt.text||'');
            },
            onBefore(file){
                let opt = this.opt;
            ...
            upload(param){
                ...
                reader.onload = (e)=>{
                    this.data[this.opt.name+'File'] = param.file;
                    this.data[this.opt.name] = e.target.result;
                }
